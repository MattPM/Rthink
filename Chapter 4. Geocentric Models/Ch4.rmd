---
title: 'Chapter 4 Geocentric Models (linear regression)'
...
Priors for h mu and sigma imply a joint prior distribution for individual heights that can be sampled from. 
...
This is a bad model -- we expect this bowtie shape because the predictions should be tighter around the mean but the large prior on beta give ridiculous expected values-smaller than 0 and larger than the tallest human ever.  

We use domain knowledge to instead assume *the logarithm of beta* has a normal distribution -- this enforces a positive relationship assuming as people get larger they get taller. If the logarithm of β is normal, it forces beta to be positive because exp(x) is greater than zero for any real number.  
...
**it is not evidence that the relationship between weight and height is linear, because the model only considered lines. It just says that, if you are committed to a line, then lines with a slope around 0.9 are plausible ones.**
...
Note:  
a better name for 'mu' would be mu.posteriors because mu is a matrix. The matrix has a value for each value of the data we asked for a prediction. The number of rows, 1000 corresponds to the default `n` parameter in `link`. The columns returned by link are whatever we supplied to the data argument. They can be the original values for the observations in the data or a random sequence of observations as below. The data argument is converted to variables. The variables are values of weight, the values are Gaussians computed using the model based on the predictor.  
...
ok so there are also infants in the dataset which explains the really small values for weight. I cant think in cm and kg which isn't helping. 
...
Im going to compare to the linear model (now again using all the data not just adults / kids) and use the information criteria to compare the models. 
Also using all data here. 
...
Model 5 gets all the weight and has a much lower deviance (-2*lppd) indicating a better fit. 
...
The model with the log realtionship is another way of expressing the log transformation of the outcome variable. 
