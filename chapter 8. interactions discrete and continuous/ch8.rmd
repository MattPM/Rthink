...
If it means the specific language of the statement is equivalent to an interaction some are not interactions.
...
8M1. Tulip example -- add temperature variable - two levels: cold and hot and assume none grew under T = hot -- explain in terms of interactions.  

B is impacted by W conditional on presence of light and W has an effect conditional on T; if T is very hot than in prioncipal the moisture in the soil might evaporate? It's an interaction because it made all blooms 0 instead of having an additive effect on blooms.  
...
8M3. Invent a dataset capturing raven population size (ravens are scavengers) depending on wolves (carnivore) for food assuming wolves tolerate raven population as 'species interaction'. Would it be linear biologically; no bc of carrying capacity and other species factors. 

Need to capture correlation in the population and also that wolf independent on Raven  
...
8H1  
tulips data -- Include bed variable as a predictor in the interaction model. Don’t interact bed with the other predictors; just include it as a main effect. Note that bed is categorical. 
...
8H3  
using the terrain ruggedness dataset evaluate impact of the outlier country seychelles on the inference.
...
The predictive ability of the robust regression model is improved while the coefficients remain largely the same: 
